...
weights_path = "C:\Users\Thameena\Desktop\sriram_traffic\yolo.weights"  # Path to YOLOv3 weights
config_path = "C:\Users\Thameena\Desktop\sriram_traffic\yolov3.cfg"       # Path to YOLOv3 config
coco_names = "C:\Users\Thameena\Desktop\sriram_traffic\coco.names"        # Path to COCO names
...
video_path1 = "C:\Users\Thameena\Desktop\sriram_traffic\V1.mp4" # Replace with your first video file path
video_path2 = "C:\Users\Thameena\Desktop\sriram_traffic\V2.mp4"  # Replace with your second video file path
...
def calculate_speed(box, current_frame):
    # Assuming a constant frame rate and distance reference
    ...
    # Example: using distance_per_frame to estimate speed
...
def calculate_combined_average_vehicle_count():
    """
    Calculate the combined average vehicle count from both video feeds.
    """
    total_count_list = vehicle_counts_during_red[0] + vehicle_counts_during_red[1]
    if len(total_count_list) > 0:
        combined_average_count = sum(total_count_list) / len(total_count_list)
        return combined_average_count
    return 0
...
def regulate_signals(vehicle_count1, vehicle_count2):
    ...
    if not red_signal[0] and current_time - last_switch_time[0] >= green_signal_duration:
        ...
        calculate_green_duration()  # Calculate the next green signal duration based on the combined count
...
def calculate_green_duration():
    """
    Calculate the green signal duration based on the combined average vehicle count during the red signal.
    """
    global green_signal_duration

    combined_average_vehicle_count = calculate_combined_average_vehicle_count()

    # Set green signal duration based on the combined average vehicle count
    if 5 <= combined_average_vehicle_count <= 10:
        green_signal_duration = 15
    elif 11 <= combined_average_vehicle_count <= 15:
        green_signal_duration = 25
    elif 16 <= combined_average_vehicle_count <= 20:
        green_signal_duration = 35
    elif 21 <= combined_average_vehicle_count <= 30:
        green_signal_duration = 40
    elif combined_average_vehicle_count >= 31:
    ...
    vehicle_counts_during_red[0].clear()
    vehicle_counts_during_red[1].clear()
...
while True:
    ...
    if ret1:
        ...
        for output in detections1:
            for detection in output:
                ...
                if confidence > 0.2 and CLASSES[classID] in ["car", "bus", "motorbike", "train"]:
                    ...
                    detected_vehicles1.append(CLASSES[classID])
                    vehicle_count1 += 1
            ...
            for output in detections2:
                for detection in output:
                    ...
                    if confidence > 0.2 and CLASSES[classID] in ["car", "bus", "motorbike", "train"]:
                        ...
                        detected_vehicles2.append(CLASSES[classID])
                        vehicle_count2 += 1

        # Update vehicle counts during the red signal
        if red_signal[0]:
        ...
        for box in detected_boxes1:
            ...
            text = f"Vehicle: {detected_vehicles1[0]}"
            cv2.putText(frame1, text, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

        for box in detected_boxes2:
            (startX, startY, endX, endY) = box
            color = [int(c) for c in COLORS[0]]
            cv2.rectangle(frame2, (startX, startY), (endX, endY), color, 2)
            text = f"Vehicle: {detected_vehicles2[0]}"
            cv2.putText(frame2, text, (startX, startY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
